...
Для создания нумерованного списка в языке разметки Markdown предусмотрен следующий механизм: перед каждым пунктом ставится числовое значение, условный номер пункта, обязательно оканчивающийся точкой. При этом не имеет значение порядок идущих чисел, ведь Markdown самостоятельно пронумерует их в правильном порядке. Так же поддерживается и вложенная нумерация списков. Для этого необходимо произвести дополнительный отсутуп после предыдущего нумерованного пункта. Например нумерованный список:
...
1. Первый пункт
2. Второй пункт
 2.1 Второй пункт, первый подпункт
 2.2 2. Второй пункт, второй подпункт
4. Специально пропущенный третий пункт всё равно окажется на своём месте
